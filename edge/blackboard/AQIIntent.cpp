...
REG_SERIALIZABLE(AQIIntent);
RTTI_IMPL( AQIIntent, IIntent );

void AQIIntent::Serialize(Json::Value &json)
{
...
void AQIIntent::Deserialize(const Json::Value &json)
{
...
void AQIIntent::Create(const Json::Value & a_Intent, const Json::Value & a_Parse)
{
	...
	Log::Debug("AQIIntent", "Parse is %s", m_Parse.toStyledString().c_str());
}